...
const longestString = (arr) => {
  let shosmo = 0;
  if (arr.length > 0) {
    let shosmo2 = arr.reduce((a, b) => {
      return a.length > b.length ? a : b;
    });
    shosmo=arr.indexOf(shosmo2); 
  }else{
    shosmo = -1;
  }
  return shosmo;
...
const firstLetters = (arr) => {
  let shosmo=arr.map((element)=>element[0]);
  return shosmo;
};
...
const findHappiness = (arr) => {
  let shosmo=arr.filter((shosmo1)=>shosmo1.includes(':)'));
  return shosmo;
};
...
const onlyOddChars = (str) => {
  let shosmo = '';
  for (let item = 1; item < str.length; item += 2) {
    shosmo += str[item];
  }
  return shosmo;
};
